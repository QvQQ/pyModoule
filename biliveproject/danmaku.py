...
class danmaku(object):
    ...
    class SEND_GIFT(msg):
        def __init__(self, stu):
            ...
            self.action = stu['data']['action']
            self.num = stu['data']['num']
            self.giftName = stu['data']['giftName']

            self.content = '{0}了{1}个{2}！'.format(self.action, self.num, self.giftName)

    class DANMU_MSG(msg):
        def __init__(self, stu):
            self.name, self.Lvlevel, self.upName, self.ULlevel, self.rank, self.brand, t, b = '','','','','','','',''
            if len(stu['info'][2]) > 2:
                self.usr = stu['info'][2][1]
            if len(stu['info'][3]) > 2:
                self.Lvlevel = stu['info'][3][0]
                self.upName = stu['info'][3][1]
            if len(stu['info'][4]) == 4:
                self.ULlevel = stu['info'][4][0]
                self.rank = stu['info'][4][3]
            if len(stu['info'][5]) > 1:
                try:
                    self.brand = danmaku.brand[stu['info'][5][1][6:]]
                except BaseException:
                    self.brand = 'unknown'
                    # with open(r'F:\yIce\Desktop\brand.txt', 'a', encoding='utf-8', errors='ignore') as f:
                    #     f.write(str(stu['info'][5], 'utf-8', 'ignore') + '\n')

            if self.upName: t= '[{0} {1}]'.format(self.upName, self.Lvlevel)
            if self.brand: b='[{}]'.format(self.brand)
            self.name = '{t}{b}[UL {u}]{name}'.format(t=t,
                                    b=b, u=self.ULlevel, name=self.usr)
            self.content = stu['info'][1]

            # with open(r'F:\yIce\Desktop\stu.txt', 'ab') as f:
    ...
    def __recv(self):
        ...
        while not self.__closed:
            ...
            if tmp == b'':
                ...
                self.__closed = True
                break
            # with open(r'F:\yIce\Desktop\ori.txt', 'ab') as f:
            #     f.write(tmp + b'\n')
                ...
                if header == self.inroom:
                    ...
                    if self.msghead == uheader[1]:  # 为一条弹幕消息
                        ...
                        if cmd == 'SEND_GIFT':  # 发送礼物
                            ...
                            # with open(r'F:\yIce\Desktop\usstu.txt', 'a', encoding='utf-8', errors='ignore') as f:
                            #     f.write(str(packet, 'utf-8', 'ignore') + '\n')
                    else:
                        print('-----------------未支持的协议头，已记录。')
                        # with open(r'F:\yIce\Desktop\pro.txt', 'a', encoding='utf-8', errors='ignore') as f:
    ...
    def __prt(self, msg):
        try:
            ...
            print('输出发生错误！已记录。')
            # with open(r'F:\yIce\Desktop\error.txt', 'a', encoding='utf-8', errors='ignore') as f:
